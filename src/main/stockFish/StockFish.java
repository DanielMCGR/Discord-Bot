 ...
 * (This is an adaptation of Rahul A R's code).
 *
 * @author Rahul A R
 * @author Daniel Rocha
 */
public class Stockfish {
    ...
    private static final String PATH = "stockfish path";

    public Stockfish () {
        if (this.startEngine()) {
            System.out.println("Stockfish Engine has started..");
        } else {
    ...
    public String getBestMove(String fen, int waitTime) {
        ...
        sendCommand("go movetime " + waitTime+" depth 25");
        System.out.println(getOutput(waitTime + (waitTime/10)));
        return "";
        //return getOutput(waitTime + 20).split("bestmove ")[1].split(" ")[0];
    }

    public String getMate(String fen, int waitTime, int moves) {
        sendCommand("position fen " + fen);
        sendCommand("go movetime " + waitTime+" mate "+moves);
        return getOutput(waitTime + 20).split("bestmove ")[1].split(" ")[0];
    }
     ...
     * @return evalScore
     */
    public String getEvalScore(String fen, int waitTime) {
        boolean isWhite = fen.split(" ")[1].equals("w");
        String white = isWhite ? "White":"Black";
        sendCommand("position fen " + fen);
        ...
        for (int i = dump.length - 1; i >= 0; i--) {
            if (dump[i].startsWith("info string NNUE ")) break;
            if (dump[i].contains("mate ")) {
                int num = Integer.parseInt(dump[i].split("mate ")[1].split(" ")[0]);
                if(bestMate!=-1) {
                    if(num<bestMate) {
                        bestMate=num;
                    }
        ...
        if(bestMate!=-1) return "Mate in "+bestMate+" for "+white;
        String values = evalScore>=0 ? "White is in the lead" : "Black is in the lead";
        evalScore/=100;
        String Eval = Float.toString(evalScore);
        return Eval+" - "+values;
    }
}
